...
authority: "用户 2026-09-03 与 2026-09-23 功能及界面决策；冲突以 2026-09-23 为准"
supersedes:
  - "离线仅指已登录桌面端短暂断网的旧范围"
  - "长计划按周展示与编辑的旧定义"
...
3. 将计划体系明确为“短计划按日、长计划按月、超长计划按年”，短计划可归档，月历可管理和套用任务存档，年度任务独立管理。
...
- 短计划保存后保持编辑并可将当前安排归档；长计划为带显式任务存档架的完整月历；超长计划为独立年度任务板，支持 12 月筛选、动态跨度条、多子任务完成/编辑/删除。
...
| D-07 | 长计划为完整月历；顶部显式展示“任务存档”架，能直接新建、编辑、删除存档，并把存档套用到一个或多个日期。短计划的当前安排可直接保存为任务存档。旧 DayPlanTemplate / weekly template 仅作内部兼容名。 |
| D-08 | 超长计划为独立的年度任务板：顶部 1–12 月按钮用于筛选任务，不跳转长计划；每个年度任务由用户编辑起止月份、颜色、说明和多个子任务。 |
| D-09 | 年度任务卡以对应月份跨度显示彩色脉冲充能条；子任务右侧提供完成确认与撤销、编辑、删除，任务本身提供编辑与删除。年度任务和长计划日历互不自动关联。 |
| D-09A | 年度任务自身另存独立 progressPercent（0–100），用户可按 +10/+5/+1 递增或直接设为 100% 完成；子任务勾选不自动覆盖任务进度。今日页另设“本月超长任务”分页，只筛选当月独立年度任务，不转换成长计划。月份导航和任务卡时间跨度条采用同一 12 列宽度与对齐基准，流光与粒子使用首尾连续的周期动画。 |
| D-09B | 年度色条提供紫、橙、蓝、绿、珊瑚红、金黄、青七种高辨识颜色，选择器直接展示色样；旧色键兼容保留，流光跟随任务本色，不再统一蓝紫化。1–12 月导航、底层灰槽和彩色跨度共用同一 12 列坐标；激活条与底槽高度、圆角和首末月边界一致。年度页面滚动时月份导航保持吸顶且继续可点击筛选。 |
| D-09C | 年度任务只保留一条充能组件：所选起止月决定与 12 月格对齐的完整空框，progressPercent 决定框内从左到右的彩色填充，0% 为空、100% 为满。取消卡片下方独立的线性进度条，保留百分比文本；+10/−10、+5/−5、+1/−1 成对控制，直接完成设为 100%，在 100% 时仍可减量回退。首页本月年度任务预览复用同一组件；子任务状态独立。此决策覆盖 D-09A 的仅增量控制和 D-09B 的全跨度彩色填充含义，不改变已存数据及接口。 |
...
| local_day_template / local_day_template_item | 从短计划保存或从零制作的任务存档，可更新、删除与重复套用。 |
| local_annual_segment / local_annual_segment_subtask | 独立年度任务及其多条子任务；保存年份、起止月份、标题、颜色、说明、完成状态和排序。 |
...
| 日序 | 中文主标题 | 英文界面 |
|---|---|---|
| 1 | 把今天，完成得漂亮。 | Make today beautifully complete. |
| 2 | 让进度，在细节里发光。 | Let progress glow in the details. |
| 3 | 清晰计划，也保留一点颜色。 | Plan clearly. Leave room for color. |
| 4 | 向前一点，就是今日亮点。 | One step forward brightens the day. |
| 5 | 把重要的事，排成好看的节奏。 | Give what matters a beautiful rhythm. |
| 6 | 今天的积累，正在变得可见。 | Today’s progress is becoming visible. |
| 7 | 轻一点开始，稳一点完成。 | Start lightly. Finish steadily. |

固定副标题语气：中文“让计划、专注与进度，在柔和色彩里清晰推进。”；英文“Let plans, focus, and progress move clearly through soft color.”
...
| 超长计划 | 月 / 年 | 12 个月筛选与年度任务卡 | AnnualPlanSegment + AnnualPlanSubtask |

周概览保留为辅助摘要和旧接口兼容，不再定义为“长计划”的主页面。用户可见名称统一为“任务存档”；代码中的 DayPlanTemplate 与 weekly template 继续映射同一份数据。年度任务单独保存，不引用月历日计划。
...
7. 短计划编辑器与计划页均提供“保存当前安排为任务存档”，编辑器可直接归档未保存的当前草稿；另有“新建空白任务存档”。存档复制任务与时段结构，不绑定当天日期和完成状态。保存后的存档可在长计划中套用、编辑和删除。
...
- 月历上方固定展示任务存档架：每个存档直接提供套用入口与编辑/删除操作；另有“新建存档”入口，能从零安排多个任务。
- 套用时在当前月选择一个或多个日期；已有计划的日期有可识别标记。
...
- 顶部固定年份、上一年、回到今年、下一年，下面排列 1–12 月按钮；点击月份筛选覆盖该月的年度任务，再点一次恢复全部任务。
- “新建年度任务”打开编辑器，用户选择起止月份（含首尾）、颜色、标题、说明，并可拆分任意多个子任务。不同任务允许月份重叠；单任务不跨自然年。
- 每个任务用一张横向卡片展示。唯一的月份充能框精确覆盖所选月份，与吸顶的 12 月按钮和灰色槽位共用等宽列、相同间距与首末边界；0% 时只显示空框，随进度在框内从左到右平滑填充，到 100% 满框。七种任务色分别用本色深浅制作光团与稀疏闪点，按周期取模移动，循环首尾无跳变；系统减少动画时保持静态。
- 不再另设任务线性进度条；持久化的 progressPercent（0–100）直接决定脉冲框内填充比例，旁边保留百分比文字。提供成对的 +10/−10、+5/−5、+1/−1 与直接完成按钮，双向钳制到 0–100%；0% 禁减、100% 禁增但可减量撤销完成。子任务完成状态与任务进度独立，编辑器可精确修改进度。首页本月年度任务预览也使用同一充能组件。
- 子任务显示名称与内容描述，右侧有完成确认/撤销按钮，也有编辑与删除入口；任务卡本身提供编辑和删除。
- 年度任务独立保存，不自动链接、跳转或展开为长计划月历与日计划；旧月卡和月轨道交互由本节替换。
- 今日计划卡及短计划页面另设“本月超长任务”分页，只显示覆盖当前月份的年度任务；进入完整年度面板可查看全年，不把年度任务转换为日计划。
...
- optional note；
- revision / updateTime。
- progressPercent（0 至 100，100 为年度任务完成；服务端旧请求省略字段时保留现值，新建默认为 0）；
- subtasks：多条 {id, title, detail, completed, sortOrder}；写入时随年度任务聚合整体提交，服务端和离线库均绑定当前用户。

### 6.5 四主题装饰与运动

12 个月按钮必须始终显示数字和选中状态。原有大型季节月卡从年度任务主视图移除；如在辅助位置使用四季图案，只能作装饰，不替代月份、任务跨度和完成语义。短计划自身的昼夜时间轴仍保留太阳/月亮。
...
- 若在辅助位置使用四季图案，四主题共用相同占位边界，切换主题不引起布局跳动。
...
| GET /api/app/v1/plans/year?year=YYYY | 返回年度任务及其子任务；兼容字段仍可返回 12 个月摘要，但新主视图不依赖该摘要。 |
| POST /api/app/v1/plans/day-templates | 新建或按名称更新任务存档。 |
| GET /api/app/v1/plans/day-templates | 获取任务存档。 |
| DELETE /api/app/v1/plans/day-templates/{id} | 删除当前用户的任务存档。 |
| POST /api/app/v1/plans/day-templates/{id}/apply-batch | 把模板幂等套用到一个或多个日期，显式提交 overwrite/skip 策略；取消发生在客户端提交前。 |
| POST /api/app/v1/plans/annual-segments | 创建年度任务及子任务列表。 |
| PUT /api/app/v1/plans/annual-segments/{id} | 原子替换当前用户年度任务的标题、月份、颜色、说明与子任务列表（含完成状态）。 |
| DELETE /api/app/v1/plans/annual-segments/{id} | 删除年度任务及其子任务。 |
...
- 再实现 AnnualPlanSegment、AnnualPlanSubtask、12 月筛选和任务卡。
- 最后接入合并月份跨度与任务进度的脉冲充能框、减少动画处理、离线仓储和同步。
...
- 短计划当前安排能保存为任务存档；长计划完整月份直接展示存档架，可新建、编辑、删除并批量套用。
- 超长计划直接显示吸顶且可操作的 12 个月按钮和独立年度任务；七种颜色选项色相鲜明，跨度条与底槽及月份列精确对齐并无缝循环；任务可 +10/+5/+1 或直接完成，多个子任务可独立确认、撤销、编辑、删除。
- 今日计划 2/2 完成后首页完成率应同步重算（包括本机离线路径）；今日页的本月超长任务分页能浏览同一份独立年度任务。
- 减少动画下年度跨度条和柔彩首页 Hero 静止，月份与完成状态仍可读取。
...
- 2026-09-23 用户补充要求覆盖“年度月份卡进入长计划”“年度区间拖动轨道”“年度目标与日计划相互引用”及只展示日模板弹出菜单的旧交互；以本文件 6.2–6.4 的任务存档架、独立年度任务卡和子任务完成操作为准。
